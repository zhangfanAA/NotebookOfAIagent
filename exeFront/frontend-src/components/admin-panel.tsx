  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-center justify-between rounded-lg border p-4">
          <div className="space-y-0.5">
            <div className="text-sm font-medium">启用 PaddleOCR（云端）</div>
            <div className="text-xs text-muted-foreground">
              启用后云端上传 PDF 支持扫描型文字识别，关闭则只支持文字型 PDF
            </div>
          </div>
          <button
            onClick={handleTogglePaddleOcrWeb}
            disabled={saving !== null}
            className={cn(
              "relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none",
              paddleOcrWebEnabled ? "bg-primary" : "bg-muted-foreground/30",
              saving === "paddle-ocr-web" && "opacity-50 cursor-not-allowed"
            )}
          >
            <span
              className={cn(
                "pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow-lg ring-0 transition-transform duration-200 ease-in-out",
                paddleOcrWebEnabled ? "translate-x-5" : "translate-x-0"
              )}
            />
          </button>
        </div>
        <div className="text-xs text-muted-foreground -mt-2">
          当前状态：{paddleOcrWebEnabled ? "已启用（支持扫描型 PDF）" : "已禁用（仅支持文字型 PDF）"}
        </div>

        {/* PaddleOCR 本地端开关 */}
        <div className="flex items-center justify-between rounded-lg border p-4">
          <div className="space-y-0.5">
            <div className="text-sm font-medium">启用 PaddleOCR（本地端）</div>
            <div className="text-xs text-muted-foreground">
              启用后本地客户端上传 PDF 支持扫描型文字识别，关闭则只支持文字型 PDF
            </div>
          </div>
          <button
            onClick={handleTogglePaddleOcrApp}
            disabled={saving !== null}
            className={cn(
              "relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none",
              paddleOcrAppEnabled ? "bg-primary" : "bg-muted-foreground/30",
              saving === "paddle-ocr-app" && "opacity-50 cursor-not-allowed"
            )}
          >
            <span
              className={cn(
                "pointer-events-none inline-block h-5 w-5 rounded-full bg-white shadow-lg ring-0 transition-transform duration-200 ease-in-out",
                paddleOcrAppEnabled ? "translate-x-5" : "translate-x-0"
              )}
            />
          </button>
        </div>
