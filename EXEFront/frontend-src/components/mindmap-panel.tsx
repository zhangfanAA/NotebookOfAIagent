  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto overflow-x-hidden">
      ...
      <div className="flex items-center justify-between border-b px-4 py-3 shrink-0">
        <h2 className="text-sm font-semibold">🗺️ 思维导图</h2>
        <div className="flex items-center gap-1">
          ...
          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setZoom((z) => Math.max(0.3, z - 0.15))} title="缩小">
            <ZoomOut className="h-3.5 w-3.5" />
          </Button>
          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setZoom((z) => Math.min(3, z + 0.15))} title="放大">
            <ZoomIn className="h-3.5 w-3.5" />
          </Button>
          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={handleExportSVG} title="导出 SVG">
      ...
      {/* Controls */}
      <div className="space-y-2 border-b p-4 shrink-0">
      ...
      {/* History list — 生成按钮下方，固定高度可滚动 */}
      {showHistory && (
        <div className="border-b shrink-0">
          <div className="flex items-center justify-between px-4 py-1.5 border-b bg-muted/30">
            <p className="text-[10px] font-medium text-muted-foreground">已保存的记录</p>
      ...
      <div
        ref={viewportRef}
        className="mindmap-viewport relative"
        onDoubleClick={handleDblClick}
      >
        {error && (
          <div className="m-4 rounded-xl border border-red-200 bg-red-50 p-3 text-xs text-red-600 dark:border-red-800 dark:bg-red-950 dark:text-red-400">
            {error}
          </div>
        )}
        ...
        {mermaidCode && !error && (
          ...
          >
            ...
            {/* 提示 */}
            <div className="absolute bottom-3 right-3 flex items-center gap-1 rounded-lg bg-background/80 backdrop-blur px-2 py-1 text-[10px] text-muted-foreground shadow-sm border">
              拖拽移动 · Ctrl+滚轮缩放 · 双击节点
            </div>
          </div>
        )}
        ...
        {!mermaidCode && !notesContent && !error && !isLoading && (
          <div className="flex flex-col items-center justify-center h-full text-muted-foreground">
            <Hand className="h-8 w-8 mb-2 opacity-30" />
            <p className="text-sm">选择文档后点击生成</p>
            <p className="text-xs mt-1">Ctrl+滚轮缩放 · 拖拽移动 · 双击节点查看解释</p>
          </div>
        )}
      </div>
